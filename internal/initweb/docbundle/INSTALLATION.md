...
Download the AhdCode v2.3.0 package for your operating system.
...
Double-click `AhdCode-2.3.0-macos-arm64.pkg` and follow the installer. The
...
Files go under `~/Library/AhdCode/versions/2.3.0`. `current` selects the
...
`AhdCode-2.3.0-macos-arm64.zip` is an alternate download that bundles the same
...
Double-click `AhdCode-2.3.0-windows-x64.exe` in File Explorer. Setup is a small
...
Files go under `%LOCALAPPDATA%\AhdCode\versions\2.3.0`. The stable command
...
`AhdCode-2.3.0-windows-x64.exe --silent` installs with no windows at all, for
...
`AhdCode-2.3.0-windows-x64.zip` bundles the same installer with the VS Code
extension.

## Linux x64

Extract `AhdCode-2.3.0-linux-x64.tar.gz` and run:

```sh
tar -xzf AhdCode-2.3.0-linux-x64.tar.gz
cd AhdCode-2.3.0
sh install.sh --setup-path
```

Files live under `~/.local/share/ahdcode/versions/2.3.0`. The installer adds
...
private Go 1.27.0, `ahdsqlite`, `ahdnumeric`, `ahdplot`, `ahdgraphics`, `ahdgui`, `ahdplotview`, and the pinned offline
