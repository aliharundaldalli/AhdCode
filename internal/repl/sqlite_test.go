...
`
	...
	Run(strings.NewReader(input), &output, &errorOutput, "AhdCode v0.3.0")
	text := output.String()
...
`
	var output, errorOutput bytes.Buffer
	Run(strings.NewReader(first), &output, &errorOutput, "AhdCode v0.3.0")
...
`
	output.Reset()
	errorOutput.Reset()
	Run(strings.NewReader(second), &output, &errorOutput, "AhdCode v0.3.0")
	if errorOutput.Len() != 0 {
