...
`
	var output, errorOutput bytes.Buffer
	Run(strings.NewReader(input), &output, &errorOutput, "AhdCode v0.6.0")
	if !strings.Contains(output.String(), "ok") {
...
func TestHTTPClientWorksInTheREPL(t *testing.T) {
	input := `bring HTTP
from HTTP bring Client
from HTTP bring ClientRequest
client := HTTP.client()
request := HTTP.clientRequest("GET", "https://example.com/")
write("ok")
`
	var output, errorOutput bytes.Buffer
	Run(strings.NewReader(input), &output, &errorOutput, "AhdCode v0.6.0")
	if !strings.Contains(output.String(), "ok") {
		t.Fatalf("REPL client output:\n%s\nerrors:\n%s", output.String(), errorOutput.String())
	}
	if errorOutput.Len() != 0 {
		t.Fatalf("REPL reported errors:\n%s", errorOutput.String())
	}
