...
func TestTimeModuleHasExactSignatures(t *testing.T) {
	tests := []struct {
		...
		{"utc produces DateTime", "current: DateTime := Time.utc()", true},
		{"timestamp produces Int", "value: Int := Time.timestamp()", true},
		{"fromTimestamp produces UTC DateTime", "value: DateTime := Time.fromTimestamp(-1)", true},
		...
		{"dateTimeUTC", "value: DateTime := Time.dateTimeUTC(year: 2026, month: 1, day: 1)", true},
		{"dateTimeOffset", "value: DateTime := Time.dateTimeOffset(year: 2026, month: 1, day: 1, offsetMinutes: 180)", true},
		...
		{"dateTimeOffset requires offset", "value: DateTime := Time.dateTimeOffset(year: 2026, month: 1, day: 1)", false},
		{"fromTimestamp rejects Real", "value: DateTime := Time.fromTimestamp(1.5)", false},
...
func TestTimeClassMembersHaveExactSignatures(t *testing.T) {
	tests := []struct {
		...
		{"offsetMinutes attribute is Int", "current: DateTime := Time.utc()\nvalue: Int := current.offsetMinutes", true},
		...
		{"timestamp produces Int", "a: DateTime := Time.now()\nvalue: Int := a.timestamp()", true},
		{"toUTC produces DateTime", "a: DateTime := Time.now()\nvalue: DateTime := a.toUTC()", true},
		{"toLocal produces DateTime", "a: DateTime := Time.utc()\nvalue: DateTime := a.toLocal()", true},
		{"toOffset produces DateTime", "a: DateTime := Time.utc()\nvalue: DateTime := a.toOffset(180)", true},
