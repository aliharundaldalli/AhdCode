...
`Math`, `Time`, `Latex`, `Word`, `Path`, `File`, `Regex`, `CSV`, `Data`, `Statistics`, `Plot`, `Numeric`, `JSON`, `XML`, and `Env` are compiler-registered and use
...
See [Time](TIME.md), [CSV](CSV.md), [Data](DATA.md), [Statistics](STATISTICS.md), [Plot](PLOT.md), [Numeric](NUMERIC.md), [Word](WORD.md), [JSON](JSON.md), [XML](XML.md), [Env](ENV.md), and the other module-specific references
