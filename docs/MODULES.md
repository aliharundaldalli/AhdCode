...
`Math`, `Time`, `Latex`, `Word`, `Path`, `File`, `Regex`, `CSV`, `Data`, `Statistics`, `Plot`, and `Numeric` are compiler-registered and use
...
See [Time](TIME.md), [CSV](CSV.md), [Data](DATA.md), [Statistics](STATISTICS.md), [Plot](PLOT.md), [Numeric](NUMERIC.md), [Word](WORD.md), and the other module-specific references
