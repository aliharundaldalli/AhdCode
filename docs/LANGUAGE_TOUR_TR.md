...
(sibling) dosyalardır. `Math`, `Time`, `Latex`, `Word`, `Excel`, `Path`, `Regex`, `CSV`, `Data`,
`File`, `Statistics`, `Plot`, `Numeric`, `JSON`, `XML`, `Env`, `Lists` ve `KeyValue` açık standart modüllerdir; alan ve dosya hataları yakalanabilir AhdCode
hatalarıdır.
...
katmanıdır; Excel tipli, değiştirilemez XLSX çalışma kitapları ekler. Ayrıca
[Time](TIME_TR.md), [CSV](CSV_TR.md), [Data](DATA_TR.md), [Statistics](STATISTICS_TR.md), [Plot](PLOT_TR.md), [Numeric](NUMERIC_TR.md), [Word](WORD_TR.md), [Excel](EXCEL_TR.md), [JSON](JSON_TR.md), [XML](XML_TR.md), [Env](ENV_TR.md), [Lists](LISTS_TR.md), [KeyValue](KEYVALUE_TR.md) ve [tanılama rehberine](DIAGNOSTICS_TR.md) bakın.
