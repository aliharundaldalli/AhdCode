...
`SQLite`, `HTTP`, `HTML`, `SMTP`, `XML`, `Env`, `Lists` ve `KeyValue` açık standart modüllerdir; alan ve dosya
...
HTTP yerel bir web sunucusudur (çerez, bellek içi oturumlar, güvenli `UploadedFile` kalıcılaştırmasıyla multipart form işleme ve giden HTTP/HTTPS Client ile), HTML küçük güvenli bir HTML oluşturucusu artı bir ayrıştırıcı ve CSS benzeri seçici sorgu yüzeyidir (`HTML.parse`, `HTMLDocument`, `HTMLElement`), SMTP yalnızca gönderim yapan postadır (`SMTPClient` / `SMTPMessage`),
